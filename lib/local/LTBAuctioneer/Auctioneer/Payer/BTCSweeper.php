...
class BTCSweeper
{
    ...
    public function __construct($native_client) {
    ...
    public function sweepBTC($source_address, $destination_address, $private_key, $float_fee) {
        ...
        $raw_tx = $this->native_client->createrawtransaction($inputs, $outputs);

        // sign the raw transaction
        $signed_tx = (array)$this->native_client->signrawtransaction($raw_tx, [], [$private_key]);
        if (!$signed_tx['complete']) { throw new Exception("Failed to sign transaction with the given key", 1); }

        // send the transaction
        $transaction_id = $this->native_client->sendrawtransaction($signed_tx['hex']);
