...
(function() {
  (function($) {
    var AuctionSocket, BID_EL_HEIGHT, FADE_SPEED, bidEntries, buildPrizeTokensAppliedList, buildReceiptsDisplay, formatCurrency, formatValueByElementSettings, generateNewBidEntry, init, initBidsByAddress, numeral, reorderBidEntries, updateAdminPageVars, updateAuction, updateBidElement, updateBids, updatePageVars;
    FADE_SPEED = 200;
    bidEntries = {};
    BID_EL_HEIGHT = 66;
    ...
    updatePageVars = function(state, auction, meta) {
      ...
      if (state.timePhase === 'ended') {
        if ($('*[data-explanation-type=live]').is(':visible')) {
          return $('*[data-explanation-type=live]').fadeOut(FADE_SPEED, function() {
            return $('*[data-explanation-type=ended]').fadeIn(FADE_SPEED);
          });
        }
      } else {
        if ($('*[data-explanation-type=ended]').is(':visible')) {
          return $('*[data-explanation-type=ended]').fadeOut(FADE_SPEED, function() {
            return $('*[data-explanation-type=live]').fadeIn(FADE_SPEED);
          });
        }
      }
    };
